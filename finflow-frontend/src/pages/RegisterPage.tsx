...
export function RegisterPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    // No backend: after "register" go back to login
    navigate('/')
  }
    ...
    <AuthLayout>
      <div className="flex w-full max-w-sm flex-col gap-6 px-8">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <button
            type="submit"
            className="w-full rounded-lg bg-indigo-600 px-4 py-2.5 font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
          >
            Register
          </button>
        </form>
